module local_halos {
    ...
    proc local_halos(in x: int, in y: int, in depth: int, in halo_depth: int, inout chunk_neighbours : [0..<NUM_FACES] int,
    in fields_to_exchange: [0..<NUM_FIELDS] bool, inout density: [?D] real, inout energy0: [?E] real,
    ...
    proc update_face (in x: int, in y: int, in halo_depth: int, inout chunk_neighbours : [0..<NUM_FACES] int, in depth: int, inout buffer: [?D] real){
    ...
    // Update left halo.
    proc update_left (in x: int, in y: int, in halo_depth: int, in depth: int, inout buffer: [?D] real) {  //possibly just use out
    ...
    // Update right halo.
    proc update_right (in x: int, in y: int, in halo_depth: int, in depth: int, inout buffer: [?D] real) {
    ...
    // Update top halo.
    proc update_top (in x: int, in y: int, in halo_depth: int, in depth: int, inout buffer: [?D] real) {
    ...
    // Update bottom halo.
    proc update_bottom (in x: int, in y: int, in halo_depth: int, in depth: int, inout buffer: [?D] real) {
        for jj in 0..<depth do { 
            forall kk in halo_depth..<x-halo_depth do{  //parallelise inner loop only
