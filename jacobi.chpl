...
module jacobi{
    ...
    ref kx: [Domain] real, ref ky: [Domain] real, ref rx: real, ref ry: real){
        ...
        const Inner_i_minus = Domain[halo_depth-1..<(y - halo_depth)-1, halo_depth..<(x - halo_depth)];
        const Inner_i_plus = Domain[halo_depth+1..<(y - halo_depth)+1, halo_depth..<(x - halo_depth)];
        const Inner_j_plus = Domain[halo_depth..<(y - halo_depth), halo_depth+1..<(x - halo_depth)+1];
        const Inner_j_minus = Domain[halo_depth..<(y - halo_depth), halo_depth-1..<(x - halo_depth)-1];
